...
class GraspProxyMatlab:
  ...
  def FitCylinder(self, cloud, viewPoints, viewPointIndices, k):
    ...
    mViewPointIndices = matlab.int32(viewPointIndices.tolist(), size=(1, len(viewPointIndices)))
    plotBitmap = matlab.logical([False, False, False, False])

    mCylinder = self.eng.FitCylinder(mCloud, mViewPoints, mViewPointIndices, k, plotBitmap)
